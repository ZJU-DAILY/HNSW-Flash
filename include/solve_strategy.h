...
class SolveStrategy {
 public:
    SolveStrategy(std::string source_path,
    ...
    void
    read_knn(std::string knn_path) {
        uint32_t num, dim;
        ReadData(knn_path, knn_results_, num, dim);
    }

    void
    save_knn(std::string knn_path) {
        WriteData(knn_path, knn_results_);
    }

    void
    recall(std::string gt_path) {
        // Read ground truth
        ...
        for (int i = 0; i < query_num_; ++i) {
            ...
            for (auto gt: gt_set[i]) {
                knn_with_dist.emplace_back(std::make_pair(
                    hnswlib::L2Sqr(org_query_set_[i].data(), org_data_set_[gt].data(), &dim), gt));
            }
            ...
            std::set_intersection(knn.begin(),
                                  knn.end(),
                                  truth_knn.begin(),
                                  truth_knn.end(),
