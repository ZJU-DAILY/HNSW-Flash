...
#include "hnswlib/nsg_flash.h"

using Eigen::MatrixXf;
using Eigen::VectorXf;

class NsgFlashStrategy: public SolveStrategy {
public:
    NsgFlashStrategy(std::string source_path, std::string query_path, std::string codebooks_path, std::string index_path):
    ...
    void solve() {
        ...
        if (std::filesystem::exists(codebooks_path_)) {
            ...
            nsg = new hnswlib::NsgFlash<data_t>(&flash_space, index_path_);
        } else {
...
#endif
            nsg = new hnswlib::NsgFlash<data_t>(&flash_space, data_num_, M_, ef_construction_);
...
#pragma omp parallel for schedule(dynamic) num_threads(NUM_THREADS)
            ...
            nsg->setCentroids(encoded_query);
            std::cout << "finished add point" << std::endl;
#pragma omp parallel for schedule(dynamic) num_threads(NUM_THREADS)
...
#pragma omp parallel for schedule(dynamic) num_threads(NUM_THREADS)
        for (size_t i = 0; i < query_num_; ++i) {
            ...
            // search
#if defined(RERANK)
            std::priority_queue<std::pair<data_t, hnswlib::labeltype>> tmp = nsg->searchKnn(encoded_query, K * 100);
