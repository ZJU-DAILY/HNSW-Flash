...
#include "hnswlib/taumg_flash.h"

using Eigen::MatrixXf;
using Eigen::VectorXf;

class TauMgFlashStrategy: public SolveStrategy {
public:
    TauMgFlashStrategy(std::string source_path, std::string query_path, std::string codebooks_path, std::string index_path):
    ...
    void solve() {
        ...
        hnswlib::TauMgFlash<data_t>* taumg;
        ...
        // This is because the encoded data is not needed after it has been added to the index.
        ...
        if (std::filesystem::exists(codebooks_path_)) {
            ...
            taumg = new hnswlib::TauMgFlash<data_t>(&flash_space, index_path_);
        } else {
#if defined(USE_PCA)
...
#endif
            taumg = new hnswlib::TauMgFlash<data_t>(&flash_space, data_num_, M_, ef_construction_);
...
#pragma omp parallel for schedule(dynamic) num_threads(NUM_THREADS)
            for (size_t i = 0; i < data_num_; ++i) {
                ...
                taumg->addPoint(encoded_data, i);
            }
            for (size_t j = 0; j < data_dim_; ++j) {
                centroids[j] /= data_num_;
            }
            uint8_t *encoded_query = thread_encoded_vector[omp_get_thread_num()];
            pqEncode(centroids.data(), encoded_query + subvector_num_ * CLUSTER_NUM * sizeof(data_t), (data_t *)encoded_query);
            taumg->setCentroids(encoded_query);
#pragma omp parallel for schedule(dynamic) num_threads(NUM_THREADS)
...
#pragma omp parallel for schedule(dynamic) num_threads(NUM_THREADS)
        for (size_t i = 0; i < query_num_; ++i) {
            ...
            // search
#if defined(RERANK)
            std::priority_queue<std::pair<data_t, hnswlib::labeltype>> tmp = taumg->searchKnn(encoded_query, K * 100);
    ...
    void pqEncode(float *data, uint8_t *encoded_vector, data_t *dist_table, int is_query = 1) {
        ...
        if (is_query == 1) {
            ...
            for (size_t i = 0; i < subvector_num_; ++i) {
                ...
                // If use AVX then change the points i that i % 4 = 1 and i % 4 = 2, to make it compat with the distance table
                if (CLUSTER_NUM == 16) {
