...
int64_t
time_cost(const std::chrono::system_clock::time_point &st,
          const std::chrono::system_clock::time_point &en) {
...
template <typename T>
void
ReadData(const std::string &file_path,
...
template <typename T>
void
WriteData(const std::string &file_path, std::vector<std::vector<T>> &results) {
...
template <typename T>
void
WriteTemp(std::vector<std::vector<T>> &results,
