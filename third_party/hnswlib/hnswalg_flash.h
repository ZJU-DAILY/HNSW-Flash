...
#define PQLINK_CALC
#define PQLINK_STORE
#define ALL_POSITIVE_NUMBER

namespace hnswlib {

#define VECTORS_PER_BLOCK 16
...
class HierarchicalNSWFlash: public AlgorithmInterface<dist_t> {
 public:
    ...
    // // data_id, subvector_id, cluster_id
    dist_table_t *flash_data_dist_table_;

    HierarchicalNSWFlash(SpaceInterface<dist_t> *s) {}
    ...
    HierarchicalNSWFlash(SpaceInterface<dist_t> *s,
        ...
        subvector_num_ = *((size_t *)s->get_dist_func_param()) * 2;
#if defined(PQLINK_STORE)
        size_links_level0_ =
            maxM0_ * sizeof(tableint) + maxM0_ * data_size_ + sizeof(linklistsizeint);
        ...
        flash_data_dist_table_ = (dist_table_t *)malloc(max_elements_ * subvector_num_ *
                                                        cluster_num_ * sizeof(dist_table_t));

#if defined(PQLINK_STORE)
        offsetLinklist_     = 0;
        offsetLinklistData_ = maxM_ * sizeof(tableint) + sizeof(linklistsizeint);
        size_links_per_element_ =
            maxM_ * sizeof(tableint) + maxM_ * data_size_ + sizeof(linklistsizeint);
...
#if defined(PQLINK_STORE)
    inline char *
    getLinksData(tableint internal_id, int level = 0) const {
        return level == 0 ? (data_level0_memory_ + internal_id * size_data_per_element_ +
                             offsetLinklistData0_)
    ...
    dist_table_t *
    get_dist_table(tableint internal_id) {
        // labeltype return_label = getExternalLabel(internal_id);
        // return flash_data_dist_table_ + subvector_num_ * cluster_num_ *
        // return_label;
        return flash_data_dist_table_ + subvector_num_ * cluster_num_ * internal_id;
    }

    void
    set_dist_table(const void *data_point, tableint internal_id) {
        // labeltype return_label = getExternalLabel(internal_id);
        // memcpy(flash_data_dist_table_ + subvector_num_ * cluster_num_ *
        // return_label, data_point, subvector_num_ * cluster_num_ *
        // sizeof(dist_table_t));
        memcpy(flash_data_dist_table_ + subvector_num_ * cluster_num_ * internal_id,
               data_point,
               subvector_num_ * cluster_num_ * sizeof(dist_table_t));
    }

    /*
        Method
        for each node, set consecutive two dimensions in one Byte
        the first 16 Bytes are "the 0th and 1st dim of node 0", "the 0th and 1st
       dim of node 1", ..., "the 0th and 1st dim of node 15" the second 16 Bytes
       are "the 2nd and 3rd dim of node 0", "the 2nd and 3rd dim of node 1", ...,
       "the 2nd and 3rd dim of node 15" and so on ... each 16 vectors set together
    ...
    */
    inline void
    setLinksData(const void *data, size_t idx, tableint neighbor_id, int level) const {
        size_t block = idx / VECTORS_PER_BLOCK;

        encoded_data_t *currObj =
            (encoded_data_t *)data + (level == 0 ? offsetLinklistData0_ : offsetLinklistData_) -
            sizeof(linklistsizeint) + block * VECTORS_PER_BLOCK * subvector_num_ / 2;
        encoded_data_t *data_point  = (encoded_data_t *)getDataByInternalId(neighbor_id);

        idx                        %= VECTORS_PER_BLOCK;
        for (size_t i = 0; i < subvector_num_ / 2; ++i) {
    ...
    searchBaseLayer(tableint ep_id, const void *data_point, int layer) {
        ...
        while (!candidateSet.empty()) {
            ...
            // #ifdef USE_SSE
            //             _mm_prefetch((char *) (visited_array + *(data + 1)),
            //             _MM_HINT_T0);
            // //             _mm_prefetch((char *) (visited_array + *(data + 1) +
            // 64), _MM_HINT_T0);
            // //             _mm_prefetch(getDataByInternalId(*datal), _MM_HINT_T0);
            // //             _mm_prefetch(getDataByInternalId(*(datal + 1)),
            // _MM_HINT_T0); #endif
            for (size_t j = 0; j < size; j++) {
                tableint candidate_id = *(datal + j);
                //                    if (candidate_id == 0) continue;
                // #ifdef USE_SSE
                //                  _mm_prefetch((char *) (visited_array + *(datal + j +
                //                  1)), _MM_HINT_T0);
                // //                 _mm_prefetch(getDataByInternalId(*(datal + j +
                // 1)), _MM_HINT_T0); #endif
...
// #ifdef USE_SSE
//             _mm_prefetch((char *) (visited_array + *(data + 1)),
//             _MM_HINT_T0); _mm_prefetch((char *) (visited_array + *(data + 1)
//             + 64), _MM_HINT_T0); _mm_prefetch(data_level0_memory_ + (*(data +
//             1)) * size_data_per_element_ + offsetData_, _MM_HINT_T0);
//             _mm_prefetch((char *) (data + 2), _MM_HINT_T0);
// #endif
#if defined(PQLINK_CALC)
            dist_t *dist_list = (dist_t *)malloc(maxM0_ * sizeof(dist_t));
            PqLinkL2Sqr(dist_list, data_point, getLinksData(current_node_id), size, 0);
#endif
            for (size_t j = 1; j <= size; j++) {
                int candidate_id = *(data + j);
                //                    if (candidate_id == 0) continue;
                // #ifdef USE_SSE
                //                 _mm_prefetch((char *) (visited_array + *(data + j +
                //                 1)), _MM_HINT_T0); _mm_prefetch(data_level0_memory_ +
    ...
    void
    getNeighborsByHeuristic2(std::priority_queue<std::pair<dist_t, tableint>,
                                                 std::vector<std::pair<dist_t, tableint>>,
        ...
        while (queue_closest.size()) {
            ...
            for (std::pair<dist_t, tableint> second_pair: return_list) {
                dist_t curdist = fstdistfunc_(get_dist_table(second_pair.second),
    ...
    get_linklist(tableint internal_id, int level) const {
        return level == 0
                   ? get_linklist0(internal_id)
                   : (linklistsizeint *)(linkLists_[internal_id] +
                                         (level - 1) * size_links_per_element_ + offsetLinklist_);
    }
        ...
        for (size_t idx = 0; idx < selectedNeighbors.size(); idx++) {
            ...
            if (!is_cur_c_present) {
                if (sz_link_list_other < Mcurmax) {
                    ...
                    dist_t d_max = fstdistfunc_(get_dist_table(cur_c),
...
#if defined(PQLINK_CALC)
                    dist_t *dist_list =
                        (dist_t *)malloc((level == 0 ? maxM0_ : maxM_) * sizeof(dist_t));
                    PqLinkL2Sqr(dist_list,
                                get_dist_table(selectedNeighbors[idx]),
...
#else
                        candidates.emplace(fstdistfunc_(get_dist_table(data[j]),
                                                        getDataByInternalId(selectedNeighbors[idx]),
                                                        dist_func_param_),
                                           data[j]);
#endif
     ...
     */
    void
    addPoint(const void *data_point, labeltype label, bool replace_deleted = false) {
        ...
        for (int layer = 0; layer <= elemLevel; layer++) {
            ...
            for (auto &&neigh: sNeigh) {
                ...
                for (auto &&cand: sCand) {
                    ...
                    //   dist_t distance =
                    //   PqSdcL2Sqr(getDataByInternalId(neigh),
    ...
    void
    repairConnectionsForUpdate(const void *dataPoint,
        ...
        if (dataPointLevel < maxLevel) {
            ...
            for (int level = maxLevel; level > dataPointLevel; level--) {
                ...
                while (changed) {
                    ...
                    tableint *datal = (tableint *)(data + 1);
                    // #ifdef USE_SSE
                    //                     _mm_prefetch(getDataByInternalId(*datal),
                    //                     _MM_HINT_T0);
                    // #endif
    ...
    addPoint(const void *data_point, labeltype label, int level) {
        ...
        memcpy(getDataByInternalId(cur_c),
               (dist_table_t *)data_point + subvector_num_ * cluster_num_,
               data_size_);
            ...
            if (curlevel < maxlevelcopy) {
                ...
                for (int level = maxlevelcopy; level > curlevel; level--) {
                    ...
                    while (changed) {
                        ...
                        PqLinkL2Sqr(
                            dist_list, data_point, getLinksData(currObj, level), size, level);
#endif
                        for (int i = 0; i < size; i++) {
                            tableint cand = datal[i];
    ...
    searchKnn(const void *query_data, size_t k, BaseFilterFunctor *isIdAllowed = nullptr) const {
        ...
        tableint currObj = enterpoint_node_;
        if (currObj > max_elements_) {
            std::cout << "currObj: " << currObj << std::endl;
        }
        if (getDataByInternalId(enterpoint_node_) == nullptr) {
    ...
    /**
     * @brief Compute L2-squared distances using PQ codebooks with SIMD (SSE) optimization.
     *
     * This function calculates the L2-squared distances between quantized vectors
     * and precomputed distance tables. It supports both SIMD (SSE) and non-SIMD fallback paths.
     *
     * @param result Pointer to the output distance array.
     * @param pVect1v Pointer to the PQ distance tables.
     * @param pVect2v Pointer to the encoded PQ vectors.
     * @param qty     Number of result elements to compute.
     * @param level   Level used to decide which graph (M0 or M) parameter to use.
     */
    void
    PqLinkL2Sqr(void *result,
                const void *pVect1v,
                const void *pVect2v,
                size_t qty,
                size_t level = 0) const {
        dist_t *res = reinterpret_cast<dist_t *>(result);

        // Determine number of blocks based on hierarchy level
        const size_t BLOCKS = (level == 0 ? maxM0_ : maxM_) / VECTORS_PER_BLOCK;

#if defined(RUN_WITH_SSE)
        // SIMD (SSE) accelerated path
        const __m128i low_mask = _mm_set1_epi8(0x0F);

        const __m128i *qdists =
            reinterpret_cast<const __m128i *>(pVect1v);                    // distance lookup table
        const __m128i *part = reinterpret_cast<const __m128i *>(pVect2v);  // encoded PQ vectors

        for (int i = 0; i < BLOCKS; ++i) {
            if (qty <= VECTORS_PER_BLOCK * i) break;

            __m128i sum32_0 = _mm_setzero_si128();
            __m128i sum32_1 = _mm_setzero_si128();
            __m128i sum32_2 = _mm_setzero_si128();
            __m128i sum32_3 = _mm_setzero_si128();

            // Process two codebooks at a time
            for (size_t j = 0; j < subvector_num_; j += 2) {
                // Load 16 packed codes (8 bits each)
                __m128i comps = _mm_loadu_si128(part);

                // Extract lower and higher 4-bit indices
                __m128i low_indices  = _mm_and_si128(comps, low_mask);
                __m128i high_indices = _mm_and_si128(_mm_srli_epi64(comps, 4), low_mask);

                // Lookup distances from PQ tables using byte-wise shuffle
                __m128i dist_l_8bit = _mm_shuffle_epi8(qdists[j], low_indices);
                __m128i dist_r_8bit = _mm_shuffle_epi8(qdists[j | 1], high_indices);

                // Expand and accumulate in 4x4 blocks
                __m128i l0 = _mm_cvtepu8_epi32(dist_l_8bit);
                __m128i r0 = _mm_cvtepu8_epi32(dist_r_8bit);
                sum32_0    = _mm_add_epi32(sum32_0, _mm_add_epi32(l0, r0));

                __m128i l1 = _mm_cvtepu8_epi32(_mm_srli_si128(dist_l_8bit, 4));
                __m128i r1 = _mm_cvtepu8_epi32(_mm_srli_si128(dist_r_8bit, 4));
                sum32_1    = _mm_add_epi32(sum32_1, _mm_add_epi32(l1, r1));

                __m128i l2 = _mm_cvtepu8_epi32(_mm_srli_si128(dist_l_8bit, 8));
                __m128i r2 = _mm_cvtepu8_epi32(_mm_srli_si128(dist_r_8bit, 8));
                sum32_2    = _mm_add_epi32(sum32_2, _mm_add_epi32(l2, r2));

                __m128i l3 = _mm_cvtepu8_epi32(_mm_srli_si128(dist_l_8bit, 12));
                __m128i r3 = _mm_cvtepu8_epi32(_mm_srli_si128(dist_r_8bit, 12));
                sum32_3    = _mm_add_epi32(sum32_3, _mm_add_epi32(l3, r3));

                part++;  // move to the next packed code vector
            }

            // Store computed 16 distances (4 blocks of 4x32-bit each)
            dist_t *res_ptr = res + (i * VECTORS_PER_BLOCK);
            _mm_storeu_si128(reinterpret_cast<__m128i *>(res_ptr), sum32_0);
            _mm_storeu_si128(reinterpret_cast<__m128i *>(res_ptr + 4), sum32_1);
            _mm_storeu_si128(reinterpret_cast<__m128i *>(res_ptr + 8), sum32_2);
            _mm_storeu_si128(reinterpret_cast<__m128i *>(res_ptr + 12), sum32_3);
        }
#else
        // -------------------------------
        // Non-SIMD fallback implementation
        // -------------------------------
        dist_table_t *pVect1   = (dist_table_t *)pVect1v;
        encoded_data_t *pVect2 = (encoded_data_t *)pVect2v;
        memset(res, 0, qty * sizeof(dist_t));

        dist_table_t *pVect1_ls, *pVect1_rs;

        for (int i = 0; i < BLOCKS; ++i) {
            pVect1_ls = pVect1;
            pVect1_rs = pVect1_ls + cluster_num_;

            for (int j = 0; j < subvector_num_; j += 2) {
                for (int k = 0; k < VECTORS_PER_BLOCK; ++k) {
                    // Accumulate distances by table lookup
                    res[i * VECTORS_PER_BLOCK + k] += static_cast<dist_t>(pVect1_ls[rs(*pVect2)]);
                    res[i * VECTORS_PER_BLOCK + k] += static_cast<dist_t>(pVect1_rs[ls(*pVect2)]);
                    pVect2++;
                }
                // Move to next pair of codebook tables
                pVect1_ls += cluster_num_ * 2;
                pVect1_rs += cluster_num_ * 2;
            }
        }
#endif
    }
